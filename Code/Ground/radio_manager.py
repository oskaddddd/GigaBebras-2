...
class Parser():
    ...
    def pack(self, format, item, start:int):
        if format in self.match_format:
            return (struct.pack(self.match_format[format], item, start)[0], self.item_length[format])
...
class radio_serial():
    def __init__(self, NET_ID, name:str = None, baud = 57600, max_packet_size = 128):
    ...
    def config_radio(self):
        ...
        EEPROM_variables_string = str(write(b'ATI5'))
        logging.debug(EEPROM_variables_string)
        ...
        for variable in EEPROM_variables_string.splitlines():
            ...
            if changed_parameters:
                ...
                out = write(b'AT&W')
                logging.debug(out)
            else: logging.debug('No parameters were changed')
    ...
    def read_packets(self, time_window = None, count = None, packet_function = None):
        ...
        sync_buffer = deque(maxlen=len(self.header_id)+1)
            ...
            if self.serial.in_waiting:
                ...
                if state == SYNC:
                    ...
                    if bytes(list(sync_buffer[:-1])) == self.header_id \
                        and sync_buffer[-1] in self.packet_structures:
